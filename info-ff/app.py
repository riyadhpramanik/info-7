...
def get_account_credentials(region: str) -> str:
    r = region.upper()
    if r == "IND":
        return "uid=4341157905&password=ARAFAT_CODEX-LRXVM3ZZW"
    elif r == "BD":
        return "uid=4341158171&password=ARAFAT_CODEX-LCZAXHR5B"
    elif r in {"BR", "US", "SAC", "NA"}:
        return "uid=4341158629&password=ARAFAT_CODEX-ZZZJOS8TK"
    else:
        return "uid=4341158973&password=ARAFAT_CODEX-F6W1D1QO9"
...
async def get_region_by_uid(uid: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"https://birthdayspecialregionapi.vercel.app/region?uid={uid}")
